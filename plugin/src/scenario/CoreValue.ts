...
async function getEtherBalance(
  ...
  let balance = await world.hre.ethers.provider.getBalance(address);

  return new NumberV(balance.toString());
}
...
const fetchers = [
  ...
  >(
    ...
    * "StorageAt addr:<Address> slot:<Number> start:<Number>, valType:<VToCastTo>" - Returns bytes at storage slot
    ...
    async (world, { addr, slot, start, valType }) => {
      ...
      const storage = await world.hre.ethers.provider.getStorageAt(
        addr.val,
        slot.toNumber()
      );
  ...
  >(
    ...
    * "StorageAtNestedMapping addr:<Address> slot:<Number>, key:<address>, nestedKey:<address>, valType:<VToCastTo>" - Returns bytes at storage slot
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      let val = await world.hre.ethers.provider.getStorageAt(addr.val, newKey);
      ...
      switch (valType.val) {
        case "marketStruct":
          ...
          let userInMarket = await world.hre.ethers.provider.getStorageAt(
            addr.val,
            newKeyTwo
          );

          let isCompKey = "0x" + toBN(newKey).add(toBN(3)).toString(16);
          let isCompStr = await world.hre.ethers.provider.getStorageAt(
            addr.val,
            isCompKey
          );
        ...
        case "list(address)":
          let p = new Array(toDecimal(val))
            ...
            .map(async (_v, index) => {
              ...
              let address = await world.hre.ethers.provider.getStorageAt(
                addr.val,
  ...
  new Fetcher<{}, NumberV>(
    ...
    async (world, {}) => {
      ...
      return new NumberV(invokation.receipt.gasUsed.toString());
    }
  ),
  ...
  >(
    ...
    async (world, { argTypes, args }) => {
      ...
      return new StringV(
        world.hre.ethers.utils.solidityPack(
