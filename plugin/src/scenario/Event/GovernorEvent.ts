...
export function governorCommands() {
  return [
    ...
    new View<{ governorArg: StringV; apiKey: StringV }>(
      ...
      `,
      "Verify",
      [new Arg("governorArg", getStringV), new Arg("apiKey", getStringV)],
      async (world, { governorArg, apiKey }) => {
        let [governor, name, data] = await getGovernorData(
          world,
          governorArg.val
        );

        return await verifyGovernor(
          world,
          governor,
          apiKey.val,
          name,
    ...
    new Command<{ governor: Governor; params: EventV }>(
      ...
      (world, from, { governor, params }) =>
        processGuardianEvent(world, governor, params.val, from),
      { namePos: 1 }
    ),
...
export async function processGovernorEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
