...
async function approve(
  ...
  let invokation = await invoke(
    ...
    from,
    comp,
    await comp.populateTransaction.approve(address, amount.encode()),
    "approve",
    NoErrorReporter
  );
...
async function transfer(
  ...
  let invokation = await invoke(
    ...
    from,
    comp,
    await comp.populateTransaction.transfer(address, amount.encode()),
    "transfer",
    NoErrorReporter
  );
...
async function transferFrom(
  ...
  let invokation = await invoke(
    ...
    from,
    comp,
    await comp.populateTransaction.transferFrom(
      owner,
      spender,
      amount.encode()
    ),
...
async function transferScenario(
  ...
  let invokation = await invoke(
    ...
    from,
    comp,
    await comp.populateTransaction.transferScenario(addresses, amount.encode()),
    "transferScenario",
    NoErrorReporter
  );
...
async function transferFromScenario(
  ...
  let invokation = await invoke(
    ...
    from,
    comp,
    await comp.populateTransaction.transferFromScenario(
      addresses,
      amount.encode()
    ),
...
async function delegate(
  ...
  let invokation = await invoke(
    ...
    from,
    comp,
    await comp.populateTransaction.delegate(account),
    "delegate",
    NoErrorReporter
  );
...
export function compCommands() {
  return [
    ...
    new Command<{ comp: Comp; recipient: AddressV; amount: NumberV }>(
      ...
      (world, from, { comp, recipient, amount }) =>
        transfer(world, from, comp, recipient.val, amount)
    ),
    ...
    new Command<{
      ...
      (world, from, { comp, owner, spender, amount }) =>
        transferFrom(world, from, comp, owner.val, spender.val, amount)
    ),
    ...
    new Command<{
      ...
      (world, from, { comp, recipients, amount }) =>
        transferScenario(
          world,
          from,
          comp,
    ...
    new Command<{ comp: Comp; account: AddressV }>(
      ...
      (world, from, { comp, account }) =>
        delegate(world, from, comp, account.val)
    ),
    new Command<{ comp: Comp; blockNumber: NumberV }>(
      ...
      * E.g. "Comp SetBlockNumber 500"
      ...
      (world, from, { comp, blockNumber }) =>
        setBlockNumber(world, from, comp, blockNumber)
    ),
  ];
}

export async function processCompEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
