...
async function genMaximillion(
  ...
  let {
    ...
    maximillion,
    maximillionData,
  } = await buildMaximillion(world, from, params);
  world = nextWorld;
...
async function verifyMaximillion(
  world: World,
...
async function repayBehalf(
  ...
  let invokation = await invoke(
    ...
    from,
    maximillion,
    await maximillion.populateTransaction.repayBehalf(behalf),
    "repayBehalf"
  );

  world = addAction(
    world,
    `Maximillion: ${describeUser(
      world,
      from
...
export function maximillionCommands() {
  return [
    new Command<{ maximillionParams: EventV }>(
      ...
      (world, from, { maximillionParams }) =>
        genMaximillion(world, from, maximillionParams.val)
    ),
    new View<{ maximillion: Maximillion; apiKey: StringV }>(
      ...
      (world, { maximillion, apiKey }) =>
        verifyMaximillion(world, maximillion, apiKey.val)
    ),
    new Command<{ maximillion: Maximillion; behalf: AddressV }>(
      ...
      (world, from, { maximillion, behalf }) =>
        repayBehalf(world, from, maximillion, behalf.val)
    ),
  ];
}

export async function processMaximillionEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
