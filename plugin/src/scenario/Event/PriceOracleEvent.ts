...
import {
  buildPriceOracle,
  setPriceOracle,
} from "../Builder/PriceOracleBuilder";
...
async function genPriceOracle(
  ...
  let {
    ...
    priceOracle,
    priceOracleData,
  } = await buildPriceOracle(world, from, params);
  world = nextWorld;
...
async function setPrice(
  ...
  return addAction(
    ...
    await invoke(
      ...
      from,
      priceOracle,
      await priceOracle.populateTransaction.setUnderlyingPrice(
        cToken,
        amount.encode()
      ),
      "setUnderlyingPrice"
...
async function setDirectPrice(
  ...
  return addAction(
    ...
    await invoke(
      ...
      from,
      priceOracle,
      await priceOracle.populateTransaction.setDirectPrice(
        address,
        amount.encode()
      ),
      "setDirectPrice"
...
export function priceOracleCommands() {
  return [
    ...
    new Command<{
      ...
      (world, from, { priceOracle, cToken, amount }) =>
        setPrice(world, from, priceOracle, cToken.val, amount)
    ),
    ...
    new Command<{
      ...
      (world, from, { priceOracle, address, amount }) =>
        setDirectPrice(world, from, priceOracle, address.val, amount)
    ),
    ...
    new View<{
      ...
      [
        ...
        new Arg("contractName", getStringV, {
          default: new StringV("PriceOracle"),
        }),
      ],
...
export async function processPriceOracleEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
