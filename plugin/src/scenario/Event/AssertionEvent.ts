...
async function assertApprox(
  ...
  if (
    Math.abs(Number(expected.sub(given).div(expected).val)) >
    Number(tolerance.val)
  ) {
    return fail(
      world,
...
async function assertEqual(
  ...
  if (!expected.compareTo(world, given)) {
    return fail(
      world,
      `Expected ${given.toString()} to equal ${expected.toString()}`
    );
  }
...
async function assertLessThan(
  ...
  if (given.compareOrder(world, expected) !== Order.LESS_THAN) {
    return fail(
      world,
      `Expected ${given.toString()} to be less than ${expected.toString()}`
    );
  }
...
async function assertGreaterThan(
  world: World,
  given: Value,
  expected: Value
): Promise<World> {
  if (given.compareOrder(world, expected) !== Order.GREATER_THAN) {
    return fail(
      world,
      `Expected ${given.toString()} to be greater than ${expected.toString()}`
    );
  }
...
async function assertFailure(world: World, failure: Failure): Promise<World> {
  if (!world.lastInvokation) {
    return fail(
      world,
      `Expected ${failure.toString()}, but missing any invokations.`
    );
  }
  ...
  if (world.lastInvokation.error) {
    return fail(
      world,
      `Expected ${failure.toString()}, but last invokation threw error ${
        world.lastInvokation.error
      }.`
    );
  }
  ...
  if (world.lastInvokation.failures.length === 0) {
    throw new Error(
      `Invokation requires success, failure or error, got: ${world.lastInvokation.toString()}`
    );
  }

  if (
    world.lastInvokation.failures.find((f) => f.equals(failure)) === undefined
  ) {
    return fail(
      world,
      `Expected ${failure.toString()}, but got ${world.lastInvokation.failures.toString()}.`
    );
  }
...
async function assertRevertFailure(
  world: World,
  err: string,
  message: string
): Promise<World> {
  ...
  if (world.lastInvokation.failures.length > 0) {
    return fail(
      world,
      `Expected revert failure, but got ${world.lastInvokation.failures.toString()}.`
    );
  }
  ...
  const expectedMessage = `VM Exception while processing transaction: ${coverageSafeRevertMessage(
    world,
    message
  )}`;

  if (
    world.lastInvokation.error.error !== err ||
    world.lastInvokation.error.errMessage !== expectedMessage
  ) {
    throw new Error(
...
async function assertError(world: World, message: string): Promise<World> {
  ...
  if (world.lastInvokation.success()) {
    return fail(
      world,
      `Expected revert, but last invokation was successful with result ${JSON.stringify(
        world.lastInvokation.value
      )}.`
    );
  }
  ...
  if (world.lastInvokation.failures.length > 0) {
    return fail(
      world,
      `Expected revert, but got ${world.lastInvokation.failures.toString()}.`
    );
  }
  ...
  if (!world.lastInvokation.error.message.startsWith(message)) {
    throw new Error(
      `Invokation error mismatch, expected: "${message}", got: "${world.lastInvokation.error.message}"`
    );
  }
...
function buildRevertMessage(world: World, message: string): string {
  return `VM Exception while processing transaction: ${coverageSafeRevertMessage(
    world,
    message
  )}`;
}
...
async function assertSuccess(world: World): Promise<World> {
  if (!world.lastInvokation || world.lastInvokation.success()) {
    ...
    return fail(
      world,
      `Expected success, but got ${world.lastInvokation.toString()}.`
    );
  }
}
...
async function assertLog(
  ...
  if (!world.lastInvokation) {
    return fail(
      world,
      `Expected log message "${event}" from contract execution, but world missing any invokations.`
    );
  } else if (!world.lastInvokation.receipt) {
    return fail(
      world,
      `Expected log message "${event}" from contract execution, but world invokation transaction.`
    );
  } else {
    const log =
      world.lastInvokation.receipt.logs &&
      world.lastInvokation.receipt.logs[event];

    if (!log) {
      const logs = Object.keys(world.lastInvokation.receipt.logs || {}).join(
        ", "
      );
      return fail(
        world,
        `Expected log with event \`${event}\`, found logs with logs: [${logs}]`
      );
    }

    if (Array.isArray(log)) {
      const found = log.find((_log) => {
        return Object.entries(keyValues.val).reduce(
...
export function assertionCommands() {
  return [
    ...
    new View<{ event: EventV; message: StringV }>(
      ...
      (world, { event, message }) =>
        assertReadError(world, event.val, message.val, true)
    ),
    ...
    new View<{ event: EventV; message: StringV }>(
      ...
      [new Arg("event", getEventV), new Arg("message", getStringV)],
      (world, { event, message }) =>
        assertReadError(world, event.val, message.val, false)
    ),
    ...
    new View<{ error: StringV; info: StringV; detail: StringV }>(
      ...
      (world, { error, info, detail }) =>
        assertFailure(world, new Failure(error.val, info.val, detail.val))
    ),
    ...
    new View<{ error: StringV; message: StringV }>(
      ...
      (world, { error, message }) =>
        assertRevertFailure(world, error.val, message.val)
    ),
...
export async function processAssertionEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
