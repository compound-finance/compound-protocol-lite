...
async function genCTokenDelegate(
  world: World,
  from: string,
  event: Event
): Promise<World> {
  const output = await buildCTokenDelegate(world, from, event);
  world = output.world;
  let cTokenDelegate = output.cTokenDelegate;
  let delegateData = output.delegateData;

  world = addAction(
    world,
    `Added cToken ${delegateData.name} (${delegateData.contract}) at address ${cTokenDelegate.address}`,
...
async function verifyCTokenDelegate(
  world: World,
  ...
  return [
    new Command<{ cTokenDelegateParams: EventV }>(
      ...
      (world, from, { cTokenDelegateParams }) =>
        genCTokenDelegate(world, from, cTokenDelegateParams.val)
    ),
    new View<{ cTokenDelegateArg: StringV; apiKey: StringV }>(
      ...
      async (world, { cTokenDelegateArg, apiKey }) => {
        let [cToken, name, data] = await getCTokenDelegateData(
          world,
          cTokenDelegateArg.val
        );

        return await verifyCTokenDelegate(
          world,
          cToken,
          name,
          data.get("contract")!,
...
export async function processCTokenDelegateEvent(
  world: World,
  event: Event,
  from: string | null
): Promise<World> {
