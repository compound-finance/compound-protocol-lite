...
export async function getProposalId(
  ...
  if (typeof proposalIdent === "string" && proposalIdent === "LastProposal") {
    ...
    return (await governor.latestProposalIds(proposer)).toNumber();
  } else {
    try {
...
async function getProposal(
  ...
  governor: Governor,
  proposalIdent: Event,
  getter: (govener: Governor, number: encodedNumber) => Promise<Event>
): Promise<Event> {
  return await getter(
    governor,
  ...
  const baseFetchers = <Fetcher<object, Value>[]>defaultedFields.map(
    ({ field, constructor, event, name, getter }) => {
      return new Fetcher<{ proposalIdent: EventV }, Value>(
        ...
        * "Governor <Governor> Proposal <Proposal> ${event}" - Returns the ${
          name || field
        } of given proposal
