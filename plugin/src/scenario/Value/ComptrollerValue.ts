...
export function comptrollerFetchers() {
  return [
    ...
    new Fetcher<{ comptroller: Comptroller; account: AddressV }, NumberV>(
      ...
      (world, { comptroller, account }) =>
        getLiquidity(world, comptroller, account.val)
    ),
    new Fetcher<
      {
        comptroller: Comptroller;
    ...
    new Fetcher<{ comptroller: Comptroller; cToken: CToken }, NumberV>(
      ...
      (world, { comptroller, cToken }) =>
        getCollateralFactor(world, comptroller, cToken)
    ),
    new Fetcher<{ comptroller: Comptroller; account: AddressV }, NumberV>(
      `
        #### MembershipLength
      ...
      (world, { comptroller, account }) =>
        membershipLength(world, comptroller, account.val)
    ),
    new Fetcher<
      { comptroller: Comptroller; account: AddressV; cToken: CToken },
      BoolV
    >(
      `
        #### CheckMembership
      ...
      (world, { comptroller, account, cToken }) =>
        checkMembership(world, comptroller, account.val, cToken)
    ),
    new Fetcher<{ comptroller: Comptroller; account: AddressV }, ListV>(
      ...
      (world, { comptroller, account }) =>
        getAssetsIn(world, comptroller, account.val)
    ),
    new Fetcher<{ comptroller: Comptroller; cToken: CToken }, BoolV>(
      ...
      (world, { comptroller, cToken }) =>
        checkListed(world, comptroller, cToken)
    ),
    new Fetcher<{ comptroller: Comptroller; cToken: CToken }, BoolV>(
      `
        #### CheckIsComped
      ...
      (world, { comptroller, cToken }) =>
        checkIsComped(world, comptroller, cToken)
    ),
    new Fetcher<{ comptroller: Comptroller }, AddressV>(
      `
        #### PauseGuardian
    ...
    new Fetcher<
      { comptroller: Comptroller; signature: StringV; callArgs: StringV[] },
      NumberV
    >(
      `
        #### CallNum
    ...
    new Fetcher<
      { comptroller: Comptroller; CToken: CToken; key: StringV },
      NumberV
    >(
      `
    ...
    new Fetcher<
      { comptroller: Comptroller; account: AddressV; key: StringV },
      NumberV
    >(
      `
        #### CompAccrued(address)
    ...
    new Fetcher<
      { comptroller: Comptroller; CToken: CToken; account: AddressV },
      NumberV
    >(
      `
        #### compSupplierIndex
...
export async function getComptrollerValue(
  world: World,
  event: Event
): Promise<Value> {
