...
function getConstructorABI(world: World, contractName: string): string {
  ...
  return constructorAbi as string;
}
...
export async function verify(
  ...
  const artifact = await world.hre.artifacts.readArtifact(contractName);
  const buildInfo = await world.hre.artifacts.getBuildInfo(
    artifact.sourceName + ":" + artifact.contractName
  );
  let sourceCode: string = await truffleFlattener(
    Object.keys(buildInfo.input.sources)
  );
  let compilerVersion: string = buildInfo.solcLongVersion.replace(
