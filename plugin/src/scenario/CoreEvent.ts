...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] =
  [
    ...
    `,
      ...
      async (world, { message }) => {
        return print(world, message.val);
      }
    ),
    new View<{}>(
      `
    ...
    new View<{
      url: StringV;
      blockNumber: NumberV;
      unlockedAccounts: AddressV[];
    }>(
      `
      #### Web3Fork

      * "Web3Fork url:<String> unlockedAccounts:<String>[]" - forks hardhat with that url
        * E.g. "Web3Fork \"https://mainnet.infura.io/v3/e1a5d4d2c06a4e81945fca56d0d5d8ea/@blocknumber\" (\"0x8b8592e9570e96166336603a1b4bd1e8db20fa20\")"
    `,
      "Web3Fork",
      [
    ...
    `,
      ...
      async (world, from, { blockNumber }) => {
        ...
        for (let i = 0; i < Number(blockNumber.val); i++) {
          await sendRPC(world, "evm_mine", []);
        }
        return world;
      }
    ...
    `,
      ...
      [new Arg("event", getEventV, { variadic: true })],
      (world, from, { event }) =>
        processUnitrollerEvent(world, event.val, from),
    ...
    `,
      ...
      [new Arg("event", getEventV, { variadic: true })],
      (world, from, { event }) =>
        processComptrollerEvent(world, event.val, from),
    ...
    `,
      ...
      [new Arg("event", getEventV, { variadic: true })],
      (world, from, { event }) =>
        processPriceOracleEvent(world, event.val, from),
...
async function getCommands(world: World) {
  ...
  const allCommands: View<any>[] = [];
  promises.forEach(async (p) => {
    const temp = await p;
    allCommands.push(temp);
  });
